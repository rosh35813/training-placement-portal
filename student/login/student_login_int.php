...
if (isset($_POST['login_student_int']))
{
    ...
    if (count($errors) == 0 ) 
    {
        // Use prepared statement and password_verify
        $stmt = $db->prepare("SELECT STUDENT_ID, S_PASSWORD, STUDENT_NAME, APPLY_FOR FROM student WHERE STUDENT_NAME = ? AND STUDENT_ID = ? AND APPLY_FOR = ? LIMIT 1");
        $stmt->bind_param('sss', $student_name, $student_id, $apply);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res && $row = $res->fetch_assoc()) {
            if (password_verify($st_password, $row['S_PASSWORD'])) {
                session_regenerate_id(true);
                $_SESSION['student_name'] = $row['STUDENT_NAME'];
                $_SESSION['success'] = "internship";
                $_SESSION['student_id'] = $row['STUDENT_ID'];
                $_SESSION['user'] = "student_int";
                header('location: ../student/dashboard/index_student_intern.php');
                exit;
            }
        }
...
<head>
    ...
    <title>Student Login - Internship | T&P SVNIT</title>
    ...
    <div class="login-card">
        ...
        <div class="card-body">
            <div class="info-badge">
                ...
                <span>Internship Portal Login</span>
            </div>

            <form method="post" action="../student/login/student_login_int.php">
                ...
                <div class="links-section">
                    ...
                    <a href="intern_forgot.php" class="forgot-link">🔑 Forgot Password?</a>
                </div>
            </form>
        </div>
    </div>
