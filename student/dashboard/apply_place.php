...
if (isset($_POST['apply_place']))
{
    ...
    if($status=='NS')
    {
        ...
        $stmt = $db->prepare("SELECT * FROM registered_placements WHERE STUDENT_ID=? AND COMPANY_NAME=? LIMIT 1");
    ...
    if (count($errors) == 0) 
    {
        $stmt = $db->prepare("INSERT INTO registered_placements (STUDENT_ID, STUDENT_NAME, COMPANY_NAME) VALUES (?, ?, ?)");
...
<head>
    ...
    <title>Apply for Placement | T&P SVNIT</title>
...
<div class="container">
    <div class="apply-card">
        <div class="card-header">
            ...
            <p>Select a company to apply for placement opportunities</p>
        </div>
        
        <div class="card-body">
            <form method="post" action="../student/dashboard/apply_place.php">
                ...
                <?php if (count($positives) == 0) : ?>
                <div class="info-box">
                    <p>ℹ️ Make sure you meet the eligibility criteria (CGPA, backlogs, attendance) before applying. You can only apply if you haven't been placed yet.</p>
                </div>

                <div class="input-group">
                    <label>Select Company for Placement</label>
                ...
                <div class="input-group">
                    <button type="submit" class="btn" name="apply_place">Submit Application</button>
                </div>
                <?php endif ?>

                <p class="footer-text">
                    <?php if (count($positives) > 0) : ?>
                        Application submitted! <a href="index_student_placement.php">Go Back to Dashboard</a>
                    <?php else : ?>
                        Changed your mind? <a href="index_student_placement.php">Go Back</a>
                    <?php endif ?>
                </p>
            </form>
        </div>
